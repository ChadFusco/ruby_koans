...
class AboutIteration < Neo::Koan
  ...
  def test_collect_transforms_elements_of_an_array
    array = [1, 2, 3]
    new_array = array.collect { |item| item + 10 } # like .map in JS.
  ...
  def test_select_selects_certain_items_from_an_array
    ...
    even_numbers = array.select { |item| (item % 2) == 0 } # analogous to .filter in JS
  ...
  def test_inject_will_blow_your_mind
    result = [2, 3, 4].inject(0) { |sum, item| sum + item } # just like JS's .reduce
